...
def scrape_match_list(start_date, end_date, sleep_time = 0.1, update_csv=True):
    ...
    if start_date <= feb_2016 <= end_date:
...
def scrape_stat_range(match_list, player_stat_range, sleep_time = 0.05, update_csv = True):
    """
    ...
    csv_file = f"data\match_player_stats_prev_{player_stat_range}.csv"
    ...
    print(f"{total_saved_new_matches} of match list already saved in {csv_file}.")
    ...
    if update_csv:
        update_csv_file(csv_file, match_player_data, ['id', 'stats'])
